...
impl fmt::Debug for Asset {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{{ polkadot:{:?}  }}", &self.priceUsd)
    }
}
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn offchain_worker(block_number: T::BlockNumber) {
            ...
            let modu = block_number.try_into().map_or(TX_TYPES, |bn| bn % TX_TYPES);
...
impl<T: Trait> Module<T> {
    ...
    fn fetch_polkadot_asset() -> Result<(), Error<T>> {
        let s_info = StorageValueRef::persistent(b"offchain-asset::polkadot");
        if let Some(Some(asset)) = s_info.get::<Asset>() {
            // gh-info has already been fetched. Return early.
            debug::info!("cached asset: {:?}", asset);
            return Ok(());
        }

        let mut lock = StorageLock::<BlockAndTime<Self>>::with_block_and_time_deadline(
            b"offchain-asset::lock",
    ...
    fn fetch_n_parse() -> Result<Asset, Error<T>> {
